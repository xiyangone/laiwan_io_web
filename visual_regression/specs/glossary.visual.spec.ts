...
test('术语表页面视觉回归且不显示语言下拉', async ({ page }) => {
    ...
    await 匹配视觉截图(page, 'glossary-page-zh');
});

test('术语表英文列表页视觉回归', async ({ page }) => {
    await 设置页面语言(page, 'en');
    await page.goto('/#/glossary');
    await 等待页面路由稳定(page);
    await expect(page.locator('select')).toHaveCount(0);
    ...
    await 匹配视觉截图(page, 'glossary-list-en');
});
...
test('术语表中文定义页视觉回归', async ({ page }) => {
    ...
    await 匹配视觉截图(page, 'glossary-definition-zh');
});
...
test('术语表英文定义页视觉回归', async ({ page }) => {
    ...
    await 匹配视觉截图(page, 'glossary-definition-en');
});
